...
| S2 — Panel de administración de usuarios | ✅ **Backend listo para producción** | `core/admin_users.py` (nuevo): `crear_usuario` (password temporal generada con `secrets`, nunca predecible, retornada UNA sola vez; `EmailDuplicadoError` case-insensitive vía CITEXT), `listar_usuarios` (nunca selecciona `password_hash`), `editar_usuario`, `desactivar_usuario` (revoca refresh tokens activos de inmediato — no espera a que expire el access token de 15 min), `resetear_password` (revoca refresh tokens + fuerza `must_change`), `actividad_usuario` (lee `auth_events`, más reciente primero). Cada escritura deja un `auth_event` con `actor_id`. `api/admin_users_endpoint.py` (nuevo): router FastAPI montado en `app/main.py`, solo rol `admin` (403 para cualquier otro rol, chequeo centralizado en un único punto de entrada), `EmailDuplicadoError`→409, `UsuarioNoEncontradoError`→404, `RolInvalidoError`→422. 13 tests nuevos (`tests/test_admin_users.py`): 8 de lógica de negocio sobre un fake de conexión Postgres (incluye email duplicado case-insensitive, revocación de refresh tokens en desactivar/reset) + 5 de HTTP con `TestClient` (403 no-admin, 409 duplicado, password temporal en la respuesta). Suite completa: 85 passed, sin regresiones. Falta (fuera de alcance, no pedido): CRUD de UI (frontend) y el drawer accesible descrito en la tabla de 13 sprints — este entregable es el backend real que reemplaza los fakes de contrato para user CRUD. |
...
| S11 — Mensajería en tiempo real (SSE) | ✅ **Listo para producción** | `GET /julix/stream` en `api/julix_endpoint.py`: mismo `JuliXService.generar_documento(...)` que `POST /julix/query`, pero transmitido fragmento a fragmento vía Server-Sent Events (eventos `chunk`/`done`/`error`). Detecta `request.is_disconnected()` en cada iteración para que el botón "Cancelar" del frontend corte el streaming de inmediato sin seguir gastando tokens. Autenticación por header o por `?token=` (EventSource no permite headers propios). 4 tests nuevos (`tests/test_julix_stream.py`) con un fake de `JuliXService`, sin tocar Anthropic. Suite completa: 63 passed (antes 52), sin regresiones. |
| S11-extra-9 — Almacenamiento de objetos para `pdf_url` | ✅ **Listo para producción** | `storage/object_storage.py` (nuevo): abstracción `ObjectStorageBackend` con `LocalStorageBackend` (por defecto, mismo comportamiento de siempre) y `S3StorageBackend` (stub listo, `boto3`, falla explícito si falta bucket/dependencia — nunca toca AWS real). `workers/pdf_worker.py:generate_pdf()` ahora sube el PDF con `get_storage_backend()` y retorna la URL final como `pdf_url` (antes guardaba la ruta local tal cual). `railway.json`: variable `OBJECT_STORAGE_BACKEND=local` en `vridik-pdf-worker`, documentado cómo activar S3. 7 tests nuevos (`tests/test_object_storage.py`), incluyendo un fake de `boto3` para el backend S3. Suite completa: 70 passed. |
| S12 — 2FA (TOTP) | ✅ **Listo para producción** | `migrations/004_totp_2fa.sql` (nueva, idempotente): columnas `totp_secret`/`totp_enabled`/`totp_activado_en` en `users`. `core/totp_2fa.py` (nuevo): flujo de activación en 2 pasos (`iniciar_activacion` genera secreto sin activar 2FA; `confirmar_activacion` exige un código válido antes de activar — nunca se activa "a medias"), `verificar_login_totp` para el segundo factor en el login, códigos de respaldo de un solo uso (hash SHA-256, nunca en claro). `pyotp` agregado a `requirements.txt`/`requirements-test.txt`. `scripts/railway_setup_rag.sh` aplica la migración. 7 tests nuevos (`tests/test_totp_2fa.py`) con códigos TOTP reales (pyotp, sin red) y un fake de conexión Postgres. Suite completa: 70 passed. Pendiente (fuera de alcance, no pedido): wiring real en un endpoint de login (S1/S2 solo existen como fakes de contrato todavía, ver `tests/support/fakes.py`). |
| S13 — Hardening y regresión final | ✅ **Listo para producción** | `api/julix_endpoint.py`: CORS explícito vía `VRIDIK_ALLOWED_ORIGINS` (nunca `"*"`, falla cerrado si no se configura), headers de seguridad en toda respuesta (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`), warning `CRITICAL` en el log si `JWT_SECRET` arranca vacío. `railway.json`: `JWT_SECRET` y `VRIDIK_ALLOWED_ORIGINS` agregados a `vridik-api.variables`. `.github/workflows/ci.yml`: los jobs `test` y `validate-sql` ahora también aplican `migrations/003_pdf_jobs.sql` y `migrations/004_totp_2fa.sql` (antes solo se corrían localmente, no en CI). 2 tests nuevos (`tests/test_api_hardening.py`). **Hallazgo de esta entrega, ya corregido:** durante la verificación se detectó que 3 archivos ya entregados en sprints anteriores (`rag/context_builder.py`, `rag/quality_gate.py`, `rag/sql/rag_chunks_schema.sql`) estaban truncados a mitad de archivo en la copia usada para verificación — se restauraron por completo a su contenido correcto (confirmado contra la fuente autoritativa) y se re-verificó `py_compile`/pytest desde un estado sin caché. Suite final completa: **72 passed, 10 skipped**, sin regresiones. |
| S11-extra-2b — Perfil real de Ana Luisa (`scripts/build_ana_profile.py`) | ✅ **Listo para producción** | Corrida autorizada explícitamente por el dev lead sobre el export real de ChatGPT (`~/Desktop/ChatGPT/`, 5 archivos `conversations-*.json`, 499 conversaciones — no ~2.204 como decía la nota pendiente en `julix/prompt_v3.txt`, ya obsoleta en ese punto). `scripts/build_ana_profile.py` (nuevo): muestra determinística de 200 conversaciones filtradas por "UGPP"/"Ana"/"pensión", cuenta indicadores de estilo **una vez por conversación** (no por mensaje, para evitar porcentajes >100%), y genera `data/ana_luisa_profile.md` — únicamente patrones agregados: nunca cédula/NIT/tarjeta profesional (redactados con regex antes de escribir cualquier fragmento ilustrativo), nunca nombres de cliente/contraparte, nunca narrativas de caso completas, ejemplos truncados a 18 palabras. Resultado real: 200 conversaciones muestreadas, 99 con mención UGPP/Ana/pensión (50%); temas — UGPP/seguridad social 80%, derecho laboral 57%, contratos y documentos corporativos 47%, actos administrativos/litigio 25%; validación de la hipótesis de estilo fija en `julix/prompt_v3.txt` — pide explicación simple 0%, rechaza/pide definir jerga 5%, pide bullets explícitos 0%, respuestas con bullets al inicio 73%, respuestas con ejemplo numérico 68%. **Recomendación del propio script:** la muestra no da señal fuerte en ninguna dirección — mantener el texto actual de `prompt_v3.txt` sin cambios hasta tener más evidencia (no se tocó `prompt_v3.txt` en este entregable). Suite completa re-verificada sin regresiones: **85 passed, 10 skipped**. |
...
crudo real de ChatGPT de Ana Luisa en `~/Desktop/ChatGPT/` (datos
...
"Estilo de respuesta" que el usuario ya había especificado.

**Actualización (S11-extra-2b, ver fila en la tabla de estado):** el usuario
autorizó explícitamente procesar ese export real para construir
`scripts/build_ana_profile.py` y generar `data/ana_luisa_profile.md`. Ya
está hecho — ver la fila `S11-extra-2b` en "Estado de sprints" para el
detalle completo (tamaño real del export, salvaguardas de privacidad,
estadísticas generadas y la recomendación resultante sobre
`julix/prompt_v3.txt`).
